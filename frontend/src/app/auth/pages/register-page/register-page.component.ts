          ...
          this.authService.me().subscribe({
            next: (user) => {
              ...
              this.authService.getMyTenants().subscribe({
                ...
                error: (err) => {
                  console.error('Error cargando tenants tras registro:', err);
                  this.toast.show(
                    'Cuenta creada, pero no se pudieron cargar los tenants.',
                    'info',
                  );
                  this.router.navigateByUrl('/select-tenant');
                },
            ...
            error: (err) => {
              console.error('Error cargando usuario tras registro:', err);

              this.authState.logout();
              this.tenantState.clear();

              this.error.set('No se ha podido cargar el usuario tras el registro.');
              this.toast.show('No se ha podido cargar el usuario tras el registro.', 'error');
            },
          });
        },
        error: (err) => {
